...
export class Dependencies {
    ...
    public static require(...templates: string[]): Promise<void> {
        const templatesToApply = templates
            .filter(template => !this.appliedTemplates.includes(template));

        return <Promise<void>><unknown>Promise.all(
            templatesToApply.map(template => this.applyTemplate(template))
        );
    }
